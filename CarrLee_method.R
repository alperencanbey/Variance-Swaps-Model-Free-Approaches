...
VarSwap_byprice_CarrLee <- function(S, puts, calls, price_put, price_call, r, T, SQ) {
...
fairvolatility_byprice_func_CarrLee <- function(input) {
  ...
  if (as.numeric(count(options)) > 0) {
    ...
    if (length(forward) == 1 & diff > 0) {
      putoptions <- subset(optiondata_filtered, optiondata_filtered$exDates == maturity 
                           ...
                           & optiondata_filtered$impl_volatility > 0)
      #you can use BS to complete implied volatilities
    ...
    fair_vol_CarrLee <- VarSwap_byprice_CarrLee(underlying, puts, calls, price_put, price_call, rate, diff, forward)$fairvol
  }
  
  else {fair_vol_CarrLee <- NA}
  ...
  DM_vol <- as.data.frame(cbind(as.Date(today), as.Date(maturity), as.numeric(fair_vol_CarrLee)))
...
date_maturity_fairvol_CarrLee<- list()
date_maturity_fairvol_CarrLee <- sapply(inputt, fairvolatility_byprice_func_CarrLee)


trio_CarrLee <- as.data.frame(unlist(date_maturity_fairvol_CarrLee))


fairvolatility_byprice_CarrLee <- matrix(NA, length(datelist), length(allmaturities)) 
rownames(fairvolatility_byprice_CarrLee) <- as.character(as.Date(datelist, format = "%Y-%m-%d"))
colnames(fairvolatility_byprice_CarrLee) <- as.character(as.Date(allmaturities, format = "%Y-%m-%d"))
fairvolatility_byprice_CarrLee <- as.data.frame(fairvolatility_byprice_CarrLee)


inputtable_CarrLee <- as.matrix(trio_CarrLee)

for (i in c(1:length(inputtable_CarrLee))) {
  input2 <- as.data.frame(inputtable_CarrLee[i,])
  ...
  fairvolatility_byprice_CarrLee[as.character(today),as.character(maturity)] <- fair_volatility
